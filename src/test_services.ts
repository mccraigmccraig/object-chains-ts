...
export interface UserServiceI {
    readonly getByIds: (d: { org_id: string, user_id: string }) =>
        Effect.Effect<never, never, User>
    readonly change: (d: { old: User, new: User }) =>
        Effect.Effect<never, never, User>
}
export const UserService =
    Context.Tag<UserService, UserServiceI>("UserService")
...
export interface PushNotificationServiceI {
    readonly sendPush: (d: { user_id: string, message: string }) =>
        Effect.Effect<never, never, string>
}
export const PushNotificationService =
    Context.Tag<PushNotificationService,
        PushNotificationServiceI>("PushNotificationService")
...
export const testServiceContext = Context.empty().pipe(
    ...
    Context.add(UserService, UserService.of({
        getByIds: (d: { org_id: string, user_id: string }) =>
            Effect.succeed({ id: d.user_id, name: "Bar" }),
        change: (d: { old: User, new: User }) => Effect.succeed(d.new)
    })),
    Context.add(PushNotificationService, PushNotificationService.of({
        sendPush: (d: { user_id: string, message: string }) =>
            Effect.succeed("push sent OK: " + d.message)
    })))
